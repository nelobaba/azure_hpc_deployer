...
# 0. Prerequisite: OPA Gatekeeper is installed (see ../commands.sh).
...
# (Confirm the webhook name/index for your Gatekeeper version first:
#   kubectl get validatingwebhookconfiguration gatekeeper-validating-webhook-configuration -o yaml | grep -A3 operations )
...
kubectl annotate namespace mine-ns "namespace-owner=$(kubectl auth whoami -o jsonpath='{.status.userInfo.username}')"
